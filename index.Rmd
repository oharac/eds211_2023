---
title: "EDS 211: Team Science"
description: "Team Science, Collaborative Analysis, and Project Management"
site: distill::distill_website
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)

# Learn more about creating websites with Distill at:
# https://rstudio.github.io/distill/website.html

# Learn more about publishing to GitHub Pages at:
# https://rstudio.github.io/distill/publish_website.html#github-pages

```

```{r, eval = FALSE, include = FALSE, out.width = "100%", fig.cap = "Update this image by dropping your new image into the img folder of the project, then open the index.Rmd & change the path to point to the new image. Image: Mississippi River south of Memphis, TN, from USGS shared on Unsplash (https://unsplash.com/photos/35Z2ylLRCO8)."}

# UPDATE IMAGE HERE 
# or copy/paste this code elsewhere, updating the file path, to add other images to your site!
knitr::include_graphics("img/usgs_sat.jpeg")
```

## Welcome to EDS 211

__Team Science, Collaborative Analysis And Project Management!__

Science in general, and data science in particular, are more and more requiring team science approaches to addressing the most pressing questions. Managing team science projects is therefore becoming an increasingly important skill for any scientist. This course will explore the principles and practical tools available for effective and efficient project management.

The course is designed to work in parallel with the beginning phase of your MEDS capstone project, focusing in particular on developing your Team Management Plan, which is due at the end of week 5.  

## Teaching team

**Instructor:** Casey O'Hara

-   **Office:** TBD
-   **Office hours:** By appointment (Slack or e-mail me!)

**EDS Group Project Team:** 

- Naomi Tague, Instructor EDS 411A
- Ruth Oliver, Instructor EDS 411B
- Jamie Montgomery, MEDS Coordinator
- Emily Case, Capstone Coordinator
- Sam Csik, Data Training Coordinator

## Important links

- [Course syllabus](https://docs.google.com/document/d/10hUvWlGR8fjW2Yt98PW00S9ngWFeJfW-8bm_ONg-s54/edit)

- [Code of Conduct](code_of_conduct.html)

## Weekly course schedule

- **Lectures:**  January 9 - February 8
    - Mondays 1:00 - 2:15 (NCEAS)
    - Wednesdays 11:00-12:15 (NCEAS)

<!-- ## Learning objectives -->

<!-- Condimentum id venenatis a condimentum. Ultricies mi quis hendrerit dolor magna eget est. Turpis egestas maecenas pharetra convallis. Sem nulla pharetra diam sit amet. Diam volutpat commodo sed egestas egestas fringilla. Mauris in aliquam sem fringilla ut morbi tincidunt augue interdum. Ornare massa eget egestas purus viverra accumsan in. -->

<!-- -   Vulputate eu scelerisque felis imperdiet proin. -->
<!-- -   Euismod nisi porta lorem mollis aliquam. -->
<!-- -   Urna molestie at elementum eu facilisis sed odio morbi quis. -->
<!-- -   Convallis a cras semper auctor neque. Semper quis lectus nulla at volutpat diam ut venenatis tellus. -->

<!-- ## Course requirements -->

<!-- Concurrent enrollment in EDS 411A. -->

<!-- ## Tentative topics -->

<!-- | Week | Lecture topics | Lab topics | -->
<!-- |------|----------------|------------| -->
<!-- | 1    | Topics         | Topics     | -->
<!-- | 2    | Topics         | Topics     | -->
<!-- | 3    | Topics         | Topics     | -->
<!-- | 4    | Topics         | Topics     | -->
<!-- | 5    | Topics         | Topics     | -->
<!-- | 6    | Topics         | Topics     | -->
<!-- | 7    | Topics         | Topics     | -->
<!-- | 8    | Topics         | Topics     | -->
<!-- | 9    | Topics         | Topics     | -->
<!-- | 10   | Topics         | Topics     | -->
